...
resource "oci_core_instance" "oracle_instance" {
  depends_on = [
    oci_core_route_table.rtb_compute,
    oci_core_network_security_group_security_rule.sg_compute_egress_all
  ]
  ...
  create_vnic_details {
    ...
    nsg_ids = [
      oci_core_network_security_group.sg_compute.id
    ]
  ...
  metadata = {
    # ssh_authorized_keys = tls_private_key.ssh_keygen_socks5.public_key_openssh
